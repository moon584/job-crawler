...
# ---------- 配置 ----------
COMPANY_ID = "CXXX"
LIST_API = "https://xxx.com/api/list"
DETAIL_API = "https://xxx.com/api/detail"   # 可选
BASE_URL = "https://xxx.com/job/detail"
REQUEST_TIMEOUT = 10
RETRY_TIMES = 2

# ---------- 辅助函数 ----------
def extract_description_requirement(data: dict):
    desc = (data.get("jobDuty") or "").strip()
    req = (data.get("jobRequirement") or "").strip()
    return desc, req

# ---------- 核心函数：独立详情获取（供 rewrite.py 调用）----------
def get_detail(post_id: str, location: str, job_url: str, job_type: int, fallback_job: dict = None) -> bool:
    """获取详情并入库"""
    # 请求详情接口（若有）
    detail_data = {}
    if DETAIL_API:
        resp = fetch_with_retry("GET", DETAIL_API, params={"id": post_id},
                                timeout=REQUEST_TIMEOUT, retry_times=RETRY_TIMES)
        if resp:
            detail_data = resp.get("data", {})

    # 提取字段（优先详情，其次 fallback_job）
    title = detail_data.get("title") or (fallback_job and fallback_job.get("title")) or "无标题"
    category = detail_data.get("category") or (fallback_job and fallback_job.get("category")) or "未分类"
    ...
    if (not description or not requirement) and fallback_job:
        ...
        description = description or desc_fb
        requirement = requirement or req_fb
    bonus = detail_data.get("bonus") or (fallback_job and fallback_job.get("bonus", ""))
    work_experience = detail_data.get("workYear") or (fallback_job and fallback_job.get("workYear", ""))
